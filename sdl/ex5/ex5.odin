...
RenderTexture :: proc(posX: f32, posY: f32, texture: ^Texture, pSrcRect: ^sdl.FRect, window: ^Window) {
    srcRect : ^sdl.FRect
    if pSrcRect == nil {
    ...
    sdl.RenderTexture(window.renderer, texture.texture, srcRect, &destinationRectangle)
}

Cleanup :: proc(app: ^App) {
    DestroyTexture(app.backgroundTexture)
...
LoadMedia :: proc(app: ^App) -> bool{
    ...
    if !LoadTexture(app, "./assets/02-textures-and-extension-libraries/foo.png", &app.characterTexture) {
        success = false
        log.info("Failed to laod the character \"foo\" texture")
    }
    if !LoadTexture(app, "./assets/02-textures-and-extension-libraries/background.png", &app.backgroundTexture) {
        success = false
    ...
    for quit == false {
        ...
        for sdl.PollEvent(event) == true {
            ...
            if keyStates[sdl.Scancode.RIGHT] {
                app.characterPosX += 1
                if app.characterPosX > f32(app.window.width) {
                    app.characterPosX = f32(app.window.width)
                }
            }
            if keyStates[sdl.Scancode.LEFT] {
                app.characterPosX -= 1
                if app.characterPosX < 0 {
                    app.characterPosX = 0
                }
            }
        }
        ...
        RenderTexture(0, 0, app.backgroundTexture, nil, app.window)
        RenderTexture(app.characterPosX, app.characterPosY, app.characterTexture, nil, app.window)
