...
UpdateText :: proc(renderer: ^sdl.Renderer, textBox: ^Text) -> bool {
	...
	textSurface: ^sdl.Surface = sdl_ttf.RenderText_Blended_Wrapped(textBox.font, ctext, 0, textBox.color, 0)
	if textSurface == nil {
...
RenderText :: proc(app: ^App, text: ^Text, dstPoint: ^Position = nil) {
	...
	if dstPoint == nil {
		...
		destinationRect.x = f32(dstPoint.x)
		destinationRect.y = f32(dstPoint.y)
	}
	RenderTexture(
		text.texture,
