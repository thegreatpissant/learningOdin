...
StartFrame ::proc(fps:^FPS) { 
	cur := sdl.GetPerformanceCounter()
	fps.fps = sdl.NS_PER_SECOND / (cur - fps.prevFrameStartTicks)
	fps.prevFrameStartTicks = fps.frameStartTicks
	fps.frameStartTicks = cur
}

EndFrame ::proc(fps:^FPS) {
	elapsedTicks := sdl.GetPerformanceCounter() - fps.frameStartTicks
	sdl.DelayPrecise(fps.targetFPSns - elapsedTicks)
}
